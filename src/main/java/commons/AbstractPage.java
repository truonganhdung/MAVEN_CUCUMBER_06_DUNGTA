...
public class AbstractPage {
	...
	public Object checkAnyImageLoaded(WebDriver driver, WebElement image) {
		JavascriptExecutor jsExecutor = (JavascriptExecutor) driver;
		return jsExecutor.executeScript("return arguments[0].complete && " + "typeof arguments[0].naturalWidth != 'undefined' && arguments[0].naturalWidth > 0", image);
	}
	...
	// public void inputToDescriptionTextBox(WebDriver driver, String
	// description) {
	// waitForControlVisible(driver, AbstractPageUI.DYNAMIC_TEXTBOX,
	// DynamicLocator.DESCRIPTION);
	//
	// if (driver.toString().toLowerCase().contains("internetexplorer")) {
	// sendkeyToElementByJS(driver, description, AbstractPageUI.DYNAMIC_TEXTBOX,
	// DynamicLocator.DESCRIPTION);
	// } else {
	// sendkeyToElement(driver, description, AbstractPageUI.DYNAMIC_TEXTBOX,
	// DynamicLocator.DESCRIPTION);
	// }
	// }
